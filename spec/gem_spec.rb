...
module GemCodebreakerAmidasd
  RSpec.describe GemCodebreaker do
    ...
    specify 'gemCodebreaker attributes' do
      ...
      expect(gemCodebreaker).to respond_to(:win)
      expect(gemCodebreaker).not_to respond_to(:win=)
      ...
      expect(gemCodebreaker).to respond_to(:array_hints)
    ...
    describe '#generate_hint' do
      ...
      it 'generates new hint and size array_hints' do
        ...
        while i <= 4
          ...
          expect(gemCodebreaker.instance_variable_get(:@array_hints).size).to be(i)
          i += 1
        end
      end
      ...
      it 'increases array_hints by one' do
        expect do
          gemCodebreaker.gets_hint
        end.to change { gemCodebreaker.instance_variable_get(:@array_hints).size }.by(+1)
      end
    ...
    describe 'guess_code' do
      ...
      it 'won' do
        gemCodebreaker.guess_code('1234')
        expect(gemCodebreaker.instance_variable_get(:@win)).equal?(true)
      end

      it 'lost' do
        gemCodebreaker.instance_variable_set(:@total_count_attempt, 0)
        gemCodebreaker.guess_code('1111')
        expect(gemCodebreaker.instance_variable_get(:@win)).equal?(false)
      end
    end
